...
const confirmValidation = (absoluteDirectory: string, filename: string): boolean => {
    ...
    const routesComponentRegex = /(export const navRoutes: RouteComponent\[] = \[\r?\n)([\s\S]*?)(\s*];)/;
...
const addPageTemplate = async (directory: string, name: string, pageType: string) => {
    ...
    if (!confirmValidation(absoluteDirectory, `${noWsName}.tsx`)) {
